...
class LocationListRepositoryImpl constructor(
    ...
    override fun getLocationList(pageIndex: Int, lat: Double, lng: Double): Flow<Response<List<LocationDataItem>>> = flow {
        object : NetworkBoundResource<List<LocationDataItem>, JResLocationList>(apiExecutor) {
            ...
            override suspend fun loadFromDb(): List<LocationDataItem> {
                return database.getLocationListDB().getDataByPageIndex(pageIndex, DEFAULT_NETWORK_PAGE_SIZE)
                        .map {//todo(move it to mapper)
                            LocationDataItem(id = it.id, name = it.name, address = it.address, city = it.city, lat = it.lat, lng = it.lng)
                        }
            }
